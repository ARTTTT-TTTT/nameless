...

{
    public class PlayerManager : CharacterManager
    {
        ...
        {
            // !ชั่วคราว แก้บัคแบบนี้ไปก่อน
            playerNetworkManager.characterName.Value = string.IsNullOrEmpty(currentCharacterData.characterName)
                ? "DefaultName" : currentCharacterData.characterName;
            ...
            // ตั้งค่าตำแหน่ง (เช็คว่าไม่ใช่ค่า (0,0,0) หรือใช้ค่าตำแหน่งเริ่มต้น)
            Vector3 myPosition = (currentCharacterData.xPosition == 0f && currentCharacterData.yPosition == 0f && currentCharacterData.zPosition == 0f)
                ? new Vector3(0f, 1f, 0f) // ค่าเริ่มต้น
                : new Vector3(currentCharacterData.xPosition, currentCharacterData.yPosition, currentCharacterData.zPosition);
            transform.position = myPosition;

            // ตั้งค่าค่าสถานะพื้นฐาน ถ้าเป็น 0 ให้กำหนดค่าเริ่มต้น
            playerNetworkManager.vitality.Value = (currentCharacterData.vitality == 0) ? 10 : currentCharacterData.vitality;
            playerNetworkManager.endurance.Value = (currentCharacterData.endurance == 0) ? 10 : currentCharacterData.endurance;

            // คำนวณค่า maxHealth และ maxStamina จากค่า vitality & endurance ที่ถูกกำหนด
            //playerNetworkManager.maxHealth.Value = playerStatsManager.CalculateHealthBasedOnVitalityLevel(playerNetworkManager.vitality.Value);
            //playerNetworkManager.maxStamina.Value = playerStatsManager.CalculateStaminaBasedOnEnduranceLevel(playerNetworkManager.endurance.Value);
            playerNetworkManager.maxHealth.Value = 1000;
            playerNetworkManager.maxStamina.Value = 100;
            // ตั้งค่าพลังชีวิตและพลังงาน ถ้าเป็น 0 ให้กำหนดให้เป็นค่าสูงสุด
